...
struct hook_trampoline_(createInstance) {
    target_offset_(0x0096B1CC) static PauseMenuDataMgr* call(void* heap) {
...
struct hook_trampoline_(lock) {
    target_offset_(0x00B1898C) static void call(sead::CriticalSection* _this) {
...
struct hook_trampoline_(unlock) {
    target_offset_(0x00B1899C) static void call(sead::CriticalSection* _this) {
        if (_this == pmdm::pmdm_mutex) {
            auto& reporter = current_reporter();
            if (!reporter.is_top()) {
...
struct hook_trampoline_(doGetItem) {
    target_offset_(0x0073A464) static void call(
        const sead::SafeString& name, const WeaponModifierInfo* modifier) {
        auto& reporter = current_reporter();
...
struct hook_trampoline_(setItemDataToPouch) {
    target_offset_(0x0073AA68) static void call(
        const sead::SafeString& name, const WeaponModifierInfo* modifier) {
        auto& reporter = current_reporter();
...
struct hook_trampoline_(loadFromGameData) {
    target_offset_(0x0096BE24) static void call(PauseMenuDataMgr* _this) {
        auto& reporter = current_reporter();
...
struct hook_trampoline_(doLoadFromGameData) {
    target_offset_(0x0096C010) static void call(PauseMenuDataMgr* _this) {
...
struct hook_trampoline_(saveToGameData) {
    target_offset_(0x0096F9BC) static void call(
        PauseMenuDataMgr* _this, const sead::OffsetList<PouchItem>& list) {
...
struct hook_trampoline_(updateInventoryInfo) {
    target_offset_(0x0096C954) static void call(PauseMenuDataMgr* _this,
                                                void* list) {
...
struct hook_trampoline_(itemGet) {
    target_offset_(0x0096EFB8) static void call(
        PauseMenuDataMgr* _this, const sead::SafeString& name, int value,
        ...
        if (modifier != nullptr) {
            reporter.sendf("args:\nname=%s\nvalue=%d\nmodifier_flags=0x%"
                           "x\nmodifier_value=%d",
                           name.cstr(), value, modifier->flags.getDirect(),
                           modifier->value);
        } else {
            reporter.sendf("args:\nname=%s\nvalue=%d\nmodifier=null",
                           name.cstr(), value);
...
struct hook_trampoline_(autoEquipLastAddedItem) {
    target_offset_(0x00970264) static void call(PauseMenuDataMgr* _this) {
...
struct hook_trampoline_(addToPouch) {
    target_offset_(0x0096F268) static void call(void* _this,
                                                const sead::SafeString& name,
                                                PouchItemType type, void* lists,
        ...
        reporter.sendf(
            "args:\nname=%s\ntype=%s\nvalue=%d\nequipped=%d\nis_load=%d",
            name.cstr(), type_str, value, equipped, is_inventory_load);
        call_original(_this, name, type, lists, value, equipped, modifier,
                      is_inventory_load);
    }
};

struct hook_trampoline_(getItemCount) {
    target_offset_(0x00970F84) static int call(PauseMenuDataMgr* _this,
                                               const sead::SafeString& name,
                                               bool count_equipped) {
        auto& reporter = current_reporter();
        auto scope = reporter.scope("pmdm::getItemCount");
        reporter.sendf("args:\nname=%s\ncount_equipped=%d", name.cstr(),
                       count_equipped);
...
struct hook_trampoline_(sub_007DC3FC) {
    target_offset_(0x007DC3FC) static void call(i64 a1) {
        auto& reporter = current_reporter();
...
struct hook_trampoline_(sub_00984CA0) {
    target_offset_(0x00984CA0) static void call(i64 a1) {
        auto& reporter = current_reporter();
...
struct hook_trampoline_(sub_00A915D4) {
    target_offset_(0x00A915D4) static i64
        call(void* a1, int a2, int a3, int a4) {
        auto& reporter = current_reporter();
...
struct hook_trampoline_(sub_0073A6D0) {
    target_offset_(0x0073A6D0) static void call(void* a1,
                                                const sead::SafeString& name) {
        auto& reporter = current_reporter();
...
struct hook_inline_(read_num_bools_s32s) {
    target_offset_(0x00DCFF14) static void call(megaton::hook::InlineCtx* ctx) {
